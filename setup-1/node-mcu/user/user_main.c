...
static bool did_set_default_route = false;
...
#define MAX_CMD_TOKENS 9
    ...
    ip_addr_t gw;
        ...
        if (nTokens == 3 && strcmp(tokens[1],"default")==0) {
        	struct netif *nif;
            for (nif = netif_list; nif != NULL; nif = nif->next) {
				if (!netif_is_up(nif)) continue;
				if (nif->name[0] == tokens[2][0] &&
	    			nif->name[1] == tokens[2][1] &&
	    			nif->num == tokens[2][2]-'0') {
	  					break;
				}
			}
			...
			if(nif == NULL)
			{
				os_sprintf_flash(response, "Invalid interface name\r\n");
			}
			else{
				netif_set_default(nif);
				os_sprintf(config.default_route, "%s", tokens[2]);
				os_sprintf_flash(response, "Default route set\r\n");
			}
			goto command_handled;
        }
...
uint32_t Bps;
	...
	//check if we can set the saved default route
	if(!did_set_default_route && os_strlen(config.default_route) > 0)
	{
		struct netif *nif;
        for (nif = netif_list; nif != NULL; nif = nif->next) {
			if (!netif_is_up(nif)) continue;
			if (nif->name[0] == config.default_route[0] &&
    			nif->name[1] == config.default_route[1] &&
    			nif->num == config.default_route[2]-'0') {
  					break;
			}
		}
		
		if(nif == NULL) os_printf("Default route found but inactive.\r\n");
		else{
			netif_set_default(nif);
			os_printf("Default route set\r\n");
			did_set_default_route = true;
		}
	}
	
	if(toggle)
		os_printf("%ld,%d,%ld,%ld,%ld,%ld,%d",
			(uint32_t)(get_long_systime()/1000000), 
			Vdd, 
			(uint32_t)Bytes_in,
			(uint32_t)Bytes_out,
			(uint32_t)Packets_in, 
			(uint32_t)Packets_out,
			(uint32_t)wifi_softap_get_station_num());
...
struct espconn *pCon;
    ...
    UART_init_console(BIT_RATE_57600, 0, console_rx_buffer, console_tx_buffer);
